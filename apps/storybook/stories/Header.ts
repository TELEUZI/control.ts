...
import { Button } from '@control.ts/design-system';
...
export const createHeader = ({ user, onLogout, onLogin, onCreateAccount }: HeaderProps) => {
  ...
  if (user) {
    ...
    account.appendChild(Button({ size: 'small', label: 'Log out', onClick: onLogout }).node);
  } else {
    account.appendChild(Button({ size: 'small', label: 'Log in', onClick: onLogin }).node);
    account.appendChild(
      Button({
        size: 'small',
        label: 'Sign up',
